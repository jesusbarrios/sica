...
class M_evaluacion_final extends CI_Model{
	...
	*/
	...
	function get_inscripcion_asignatura($facultad = false, $sede = false, $carrera = false, $semestre = false, $asignatura = false, $periodo = false, $persona = false){
		...
		if($periodo && $facultad && !$sede){
			$this->db->group_by('t1.id_sede');
			$this->db->order_by('t1.id_sede', 'asc');
		}

		if($periodo && $facultad && $sede && !$carrera){
			$this->db->group_by('t1.id_carrera');
			$this->db->order_by('t1.id_carrera', 'desc');
		}

		if($periodo && $facultad && $sede && $carrera && !$semestre){
			$this->db->group_by('t1.id_semestre');
			$this->db->order_by('t1.id_semestre', 'asc');
		}

		if($periodo && $facultad && $sede && $carrera && $semestre && !$persona){
