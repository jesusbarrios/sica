    	...
    	$(document).ready(function () {
		  ...
		  if ($("#mensaje").length > 0){
    		...
    		$('#btn_buscar').click(function() {
	    		...
	    		$.post('<?=base_url()?>index.php/inscripcion/evaluacion_final/autocompletar_campos', {txt_documento : documento}, function (respuesta) {
//	    			alert(respuesta);
					...
					if(datos['apellido']){
						...
						$('#slc_oportunidad').html(datos['oportunidades']).attr('disabled', true);
						$('#btn_buscar').prop( "disabled", true );
						$('#detalles').html('');
					}else{
    		...
    		$('#slc_sede').change(function() {
    			documento = $('#txt_documento').val();
    			sede = $('#slc_sede').val();
    			$('#slc_oportunidad').attr('disabled', true).val('NULL');
	    		$.post('<?=base_url()?>index.php/inscripcion/evaluacion_final/actualizar_slc_carrera', {txt_documento : documento, slc_sede : sede}, function (respuesta) {
					var datos = $.parseJSON(respuesta);
//					alert(respuesta);
					$('#slc_carrera').html(datos['carreras']);
					$('#slc_semestre').html(datos['semestres']);
    		...
    		$('#slc_carrera').change(function() {
	    		...
	    		$.post('<?=base_url()?>index.php/inscripcion/evaluacion_final/actualizar_slc_semestre', {slc_sede : sede, slc_carrera : carrera, txt_documento : documento}, function (respuesta) {
    		...
    		$('#slc_semestre').change(function() {
	    		$('#detalles').html('');
	    		$('#slc_oportunidad').val('NULL')
	    		
	    		if($(this).val() != 'NULL')
	    			$('#slc_oportunidad').focus().attr('disabled', false);
	    		else
	    			$('#slc_oportunidad').attr('disabled', true);
    		});
    		...
    		$('#slc_oportunidad').change(function() {
    			...
    			if(oportunidad){
		    		$.post('<?=base_url()?>index.php/inscripcion/evaluacion_final/cargar_detalles', {slc_sede : sede, slc_carrera : carrera, slc_semestre : semestre, slc_oportunidad : oportunidad, txt_documento : documento}, function (respuesta) {
//		    			alert(respuesta);
    		...
    		$('#btn_cancelar').click(function() {
    			...
    			$.post('<?=base_url()?>index.php/inscripcion/evaluacion_final/cancelar', function (r) {
	    			alert(r)
    			})	    		
    		});
	});
...
<?php
	echo form_open();
		echo form_fieldset('Inscripción a evaluación final');
...
//			echo "<div class='frm_alert' id='mensaje'>Esta inscripción ya existe.</div>";
			...
			echo "<div class='frm_ok' id='mensaje'>La actualizacion fue exitosa.</div>";
